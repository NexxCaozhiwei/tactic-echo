...
本规范用于 Tactic Echo `1.0.44 P5` 之后的需求、实现、验证和交付管理。

## 版本治理

- 当前唯一开发基线为 `1.0.44 P5`。
- 每次代码修改必须保持根目录 `VERSION`、AddOn TOC、`addon/!TacticEcho/Core/Bootstrap.lua` 版本一致。
- 当前版本根基线、补丁清单和本目录文档必须能互相解释，不得出现相互矛盾的运行时规则。
...
- `doc/`：当前项目管理、基线和交付规则入口。
- 根目录 `BASELINE_1.0.44.md`：当前版本基线入口。
- `doc/baselines/BASELINE_*.md`：历史版本基线归档。
- 根目录 `PATCH_MANIFEST_*.md`：版本覆盖/交付清单历史。
