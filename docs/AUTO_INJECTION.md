...
当技能存在天赋、英雄天赋或覆盖形态时，声明 SpellID 可能继续返回旧的基础冷却，而当前绑定动作已经就绪。只有 Resolver 已验证的直接默认动作槽位同时给出可解释的 `0/0` 且明确不在 GCD 时，自动注入才允许以该动作槽位纠正旧 SpellID 的自身冷却；宏、间接来源和不可信槽位仍然 fail-closed。若动作条布尔状态与 `0/0` 数值矛盾，纠正结果会写入纯标量诊断，供实机日志复核。

若官方前置窗口恰好在可信直接动作仍报告最后一小段自身冷却时出现，1.4.9 会保留原 pre-window capture 做有界复检。预算为受限客户端 `SpellQueueWindow + 0.75s`，最少 0.85 秒、最多 1.20 秒，用于覆盖官方窗口与真实动作 ready 边沿之间的实测采样抖动；期间只输出 observation-only、`BindingToken=0`。同一动作明确 ready 后才创建原顺序计划，持续冷却、窗口离开、绑定变化、脱战或预算到期均恢复原 simple/focused 安全行为。宏、饰品、窗口后步骤和不可信动作槽位不进入该边沿复检。
