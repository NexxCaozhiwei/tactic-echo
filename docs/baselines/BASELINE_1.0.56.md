# Tactic Echo 1.0.56 P5.11 基线：共享宏资格与控制诊断收口
...
- 当同一有效 numeric macro index 的当前动作条文本明确包含请求技能名、但同 index 正文暂不可读时，可显示该**同一按钮**的只读失败记录，便于定位 `CTRL+1` 宏状态；该记录不授权映射、不触发跨宏扫描。
