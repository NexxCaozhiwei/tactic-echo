...
1.1.0 是审计后的当前基线，延续 P5.11 的共享宏资格，并保持自动打断硬暂停与脱战 Burst 硬门控；HUD 主键可启动/暂停。爆发、打断、控制、防御与生存共享同一套“当前可见按钮/槽位 + 当前宏身份 + 正文语义”资格规则。已确认的 `/cast`、`/use`、条件、`@focus`、`@mouseover`、`@cursor`、目标管理和 `/castsequence` 宏保持原有行为；控制、防御、生存只保留 `BindingToken=0` 的 HUD 人工点击能力。`action_info_*` 正文不可读兼容仅保留 P4 target-only reaction transport。无关“坐骑”等宏不再被显示为控制技能候选。

Tactic Echo 是一套以游戏内 AddOn 观察与既有动作条映射为前提、通过 TEAP 与 TEK 受控交付官方推荐与自动爆发，同时提供只读打断提示和 HUD 人工点击入口的战术辅助项目。

## 当前自动打断状态

1.1.0 中自动打断设计继续硬暂停。设置项保持可见但不可选择；Defaults、SavedVariables 规范化与运行时都会强制 `auto_interrupt_suspended`。读条 API、事件、钢条证据、冷却采样、动作条/宏解析继续服务于只读提示、高亮与诊断，不会生成 reaction candidate、BindingToken、TEAP reaction 帧或 TEK 自动打断请求。
...
- `docs/baselines/BASELINE_1.1.0.md`：1.1.0 审计基线、自动打断硬暂停与共享宏资格冻结；
