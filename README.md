...
- TEAP freshness 仍以 50ms 刷新供 TEK 采样；官方主推荐读取、动作条 BindingToken 解析和常规官方派发 sequence 参考 WR，默认每 `0.10s` 重建一次。Burst/Reaction 候选不使用该官方帧缓存。

## HUD 展示规则

- HUD 底部蓝色状态行以派发许可为准：当前帧仍允许 TEAP/TEK 派发时显示 `HAD`。
...
- `TacticalBoard` 容器在战斗中不得调用 `SetScale`、`SetAlpha`、`SetShown`、`Show`、`Hide`、`StartMoving` 或 `StopMovingOrSizing`。
- `TacticalHudLayout` 在战斗中不得执行 `SetScale`、`SetPoint`、`SetSize`、`SetShown` 等布局变更，只能记录 pending/dirty，脱战后再真实应用；HUD 拖拽移动同样只能脱战真实执行。
...
- TEAP catalog 是通用空目录，不保存职业或专精动作表。
- 常规官方主推荐帧使用 `actionCode=0`；TEK 只按非零 `BindingToken` 解码和派发。
- 不得恢复惩戒骑或其他职业专用 ActionRegistry / 默认 profile 绑定表作为派发前置条件。
