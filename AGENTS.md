# AGENTS.md — Tactic Echo 1.5.0 开发与安全边界
...
- `simple`：初始活动链移除明确自身 CD、无法经公开 GCD 阶段安全降级派发的 UNKNOWN、无绑定、未装备、装备身份变化或其他硬失效步骤；明确自身 CD 且绑定/装备身份仍有效的步骤保留为候补，只有位于当前配置游标之后时才可在恢复后按原位置补入；已越过步骤永久失效，剩余步骤相对顺序不变。
- `focused`：任何已启用可选步骤存在明确自身 CD、无绑定、未装备、装备变化或其他硬失效时不创建计划；单帧资源不足可进入运行期稳定确认，通用临时不可用必须保留原步骤等待恢复。
- 单帧资源不足、通用不可用或已验证动作栏上可经公开 GCD 阶段继续派发的冷却来源不确定不得删除可选注入。明确资源不足仍需两个不同共享业务快照稳定确认；通用 `usable=false` 且 `notEnoughResource=false` 只能标记为 `TEMPORARILY_UNUSABLE`，预检保留步骤，运行期输出 `BindingToken=0` 的 `burst_step_wait_usable`，恢复可用后继续同一 plan/group/stepIndex。匹配失败后观察到该临时状态时，该失败不得参与两次失败释放证书。普通读条、引导、蓄力以及 `GCD_LOCKED` / `QUEUE_WINDOW` 期间不得把 `usable=false` 解释为资源不足；明确自身 CD、绑定/装备变化与生命周期硬终止仍优先，临时状态不得遮蔽自身 CD。噬灭虚空变形的 `usable=false/false` 兼容仍受其专用范围约束。
- 所有可选步骤均被移除时，不创建计划。
...
- UNKNOWN、图标灰度、泛 GCD、单个失败事件、Buff、资源/目标状态不得作为跳过或成功证据。当前 `WAIT_CONFIRM` 精确 SpellID/等效 SpellID 的匹配 `UNIT_SPELLCAST_FAILED`、`UNIT_SPELLCAST_FAILED_QUIET` 或 `UNIT_SPELLCAST_INTERRUPTED` 收据只为已锁定可选步骤建立无 Token 隔离与同一步重试，不得按次数删除该步骤；失败时明确观察到玩家移动则保持移动等待，停下后再进入隔离重试。仅窗口步骤仍允许两个匹配失败形成有界活性释放；不得用候选帧数、固定睡眠或泛超时替代该精确失败证据。
