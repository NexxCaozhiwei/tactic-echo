# AGENTS.md — Tactic Echo 1.1.7 开发与安全边界
...
- HUD 底部蓝色状态行必须以 `primary.dispatchAllowed == true` 为最高优先级显示 `HAD`，即当前帧仍允许 TEAP/TEK 派发就显示 `HAD`；HUD 按钮自身不得把 dispatchable 状态改写为 `HAD`，按钮状态/tooltip fallback 仍沿用原有“可用/推荐/施法/引导”等语义。
...
- 官方主推荐输入链路必须对所有职业/专精一致：不得恢复惩戒骑或任何职业专用 ActionRegistry、动作码、默认 profile 绑定表作为派发前置条件。TEAP catalog 只保留通用空目录握手；常规官方主推荐帧 `actionCode=0` 是正常状态，TEK 只按非零 `BindingToken` 解码和派发。
...
- SignalFrame 必须区分 TEAP freshness 与昂贵推荐构建：TEAP 色块继续 50ms 刷新 freshness 供 TEK 采样；官方主推荐读取、动作条 BindingToken 解析和常规官方派发 sequence 推进参考 WR，默认 `0.10s` 重建一次。Burst/Reaction 候选不得使用该官方帧缓存，活跃 AutoBurst 计划仍按既有 TEAP tick 评估。
