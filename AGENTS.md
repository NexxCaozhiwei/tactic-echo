# AGENTS.md — Tactic Echo 1.5.7 开发与安全边界

## 0. 当前收窄范围（最高优先级）

- 当前产品范围只保留：首页/设置中心、HUD 主键、官方主推荐输入链路、AutoBurst 及其设置页/运行链路。
- 打断、控制、防御、生存模块，包括自动打断、自动控制、只读反应高亮、姓名板群控扫描、目标框/姓名板打断提示、TargetCastPrompt、监控/调试页面、MappingExport、OfficialApiProbe，均视为退役功能，默认不得加载、不得轮询、不得在 HUD 中显示，也不得通过设置页重新启用。
- HUD 只允许展示主键与 AutoBurst 队列；候选历史队列、打断/控制/位移、防御/生存卡片必须保持空输出或隐藏。`queueMode=primary` 表示仅主键；其他旧队列模式只能解释为“主键 + 爆发”。
- HUD 可见期间，底部状态行常驻自动注入模式 `HAD`（开启）或 `LCC`（关闭），并独立显示可用、暂停、待命等运行状态；LCC 仍显示已启用组的只读注入卡片，不产生自动执行资格；HUD 按钮自身不得把 dispatchable 状态改写为 `HAD`，按钮状态/tooltip fallback 仍沿用原有“可用/推荐/施法/引导”等语义。
- 灰条施法展示必须保持只读：普通施法显示“施法”，引导显示“引导”，射击猎等射击类灰条施法显示“射击”。该判断只能依赖安全透传的当前施法 spellID、等效 spellID、基础技能一致性或施法名称，不得影响推荐、BindingToken、TEAP、TEK 或任何派发资格。
- BBA 风格按下反馈只允许在 `UNIT_SPELLCAST_SUCCEEDED` 等游戏确认事件后触发，表达技能最终被使用；不得恢复鼠标按下即显示按下效果的旧行为。
- 以下历史章节中关于打断、控制、防御、生存、调试导出、P3/P4/P5 反应链路的要求，只保留为归档背景；如与本节冲突，以本节为准。

## 1. 基线与交付

- 当前唯一开发基线：`1.5.7`。用户可见的 AutoBurst 已泛化并更名为“自动注入”：每专精最多三个独立配置组必须共享唯一 `AutoInjectionCoordinator`、唯一 AutoBurst OrderedPlan、唯一 capture 和唯一候选；不得复制并行状态机、事件、轮询或输入路径。新组必须保持关闭并允许先完整编辑窗口 SpellID、注入/饰品和顺序，配置完整后才可启用；本组窗口不得再次作为本组注入。HUD 必须按组保存顺序展示全部已启用组，每组保持自身步骤顺序并与其他组分行或分列，最多三组、每组九张、总计 27 张只读卡；这不得改变唯一活动组和唯一派发计划。当前组启用/停用入口必须位于“当前组基本设置”上方；窗口字段须说明只填写数字 SpellID，周期刷新不得覆盖当前组正在编辑的名称、窗口或其他文本输入；自动注入页滚动高度须跟随当前实际序列行数，不得保留固定 3200 像素尾部空白。AutoBurst 与输入路径仍以 1.0.35 为冻结基线；HUD 冷却渲染仍以已实测稳定的 1.0.31 实时转盘路径为基础，并由 1.0.38 收口为“DurationObject 只画转盘、HUD 徽标统一纯秒数”。自动打断生产运行继续硬暂停，自动注入保持脱战硬门控。HUD 卡片与 HUD 手动点击层在战斗中不得直接调用受保护 `Button:Hide()` / secure proxy Hide 路径；卡片 Button 在战斗中不得调用 `SetAlpha`、`EnableMouse`、`Show` 或 `Hide`，只能记录待处理状态；`HudClickRouter` 的 secure proxy 与 blocker 在战斗中同样不得调用 `SetAlpha`、`Show` 或 `Hide`，只能记录 `tacticEchoCombatVisibilityPending` 与 `dirty`；`TacticalBoard` 与 defense 容器在战斗中不得调用 `SetScale`、`SetAlpha`、`SetShown`、`Show` 或 `Hide`，`TacticalHudLayout` 在战斗中不得执行 `SetScale`、`SetPoint`、`SetSize`、`SetShown` 等布局变更，只能记录 pending/dirty，脱战后再真实隐藏、显示、缩放或重建。TEK 本地连发介入白名单仅豁免配置中的主键，默认 `W/A/S/D/SPACE` 不触发手动让步；非白名单真实键盘输入，包括不能单独加入白名单的 `Ctrl`、`Alt`、`Shift`、`Win` 修饰键，必须从按下到抬起持续 `manual_input_held`，抬起后再进入既有 release delay、freshness 与 replay guard。TEK 必须支持 `Shift+1` 至 `Shift+=` 等动作条绑定 token；扩展修饰键或新键位时必须同步 binding token 解析、SendInput 编码与测试。默认 `pause_out_of_combat` 策略对外命名为“自动启停”：未进战斗或脱战时保持底层 `paused` 安全帧，但 HUD/设置中心必须显示“待命”，进战后自动恢复运行；手动暂停和脱战停止仍显示为暂停。无论 Defaults、旧 SavedVariables、组配置、官方窗口、已有 plan/capture 或既有 bridge 字段为何，只要 `inCombat=false`，不得创建/保留 Burst capture 或 plan，不得生成 Burst candidate、TEAP Burst 帧或 TEK 请求。默认动作条槽位可作为 TEAP/TEK 自动派发来源扫描，不要求对应动作条按钮当前可见；HUD 手动点击只能静态 secure proxy 复用可靠当前可见默认动作条按钮/已识别宏，隐藏按钮只能用于 TEAP/TEK 自动派发，不能成为 HUD 手动点击代理。HUD/原生默认动作条真实左键必须以 `manual_hold`、动作码 0、BindingToken 0 优先于后续派发。宏列表存在、宏名、图标及未知宏正文均不得作为当前动作栏身份或钢条判断依据。此前版本仅保留历史记录。 1.4.5 进一步冻结：Claim 只代表已建立的 plan、pre-window capture 或 departure lock 真实所有权；每个 armed epoch 的前置窗口只允许首次健康观察恢复一次；group runtime 必须使用明确 `<profileKey>:<groupId>` 且不得产生 `unknown:<groupId>`；所有冷却/充能回退（包括饰品）必须经过至少两个样本与 0.15 秒稳定确认；非法启用组 HUD 必须显示 INVALID 且无派发暗示；高频 Evaluate 不得逐帧执行完整 Normalize；persistent recovery 策略与 TEAP/TEK 协议不变。 1.4.6 进一步冻结：departure lock 直接观察到另一组窗口时必须先以旧所有权记录新窗口错过再释放；任何已拥有 capture 的 Abort 都必须先建立必要离开锁；冷却/充能回退在 runtime cycleId 可用时必须来自两个不同业务样本；所有手工建锁/无锁终止路径必须同步 Claim/Release，释放原因不得覆盖忽略事件。 1.4.14 进一步冻结：每个计划只允许一个配置顺序游标；初始自身 CD 步骤仅可作为未越过位置的候补，恢复后按原始位置向后补入，不得重建或重放前缀；已进入活动链的可选步骤不得因匹配失败次数被删除，移动失败期间只允许无 Token 等待并在停下后重试。 1.5.1 进一步覆盖：任何步骤首次派发必须有明确自身可用证据，UNKNOWN 或明确自身 CD 不得借公开 GCD 降级派发；已经明确接纳的当前步骤可在后续读数短暂 UNKNOWN 或移动失败时继续共享节奏；施法、引导、蓄力期间不得选择规则、扫描候补或推进游标。 1.5.2 进一步覆盖：配置窗口在普通施法、引导或蓄力期间必须以 observation-only、BindingToken 0 身份栅栏阻止官方链路提前派发，但不得选择规则、创建 plan/capture、扫描候补或推进游标；当前步骤只有在精确成功或稳定自身 CD/充能证据确认后才推进；推进前若存在配置缺口，必须等待不同 runtime cycleId 的下一业务快照，再按原 `configuredIndex` 接纳明确可用步骤或永久越过仍为 CD/UNKNOWN 的位置；已经越过的位置不得回插到已派发步骤之前。 1.5.3 进一步覆盖：已派发步骤在施法、引导或蓄力保护期间进入 `SOFT_PAUSED` 时，只要 `pauseResumeState=WAIT_CONFIRM` 且当前 wait 身份仍一致，必须接收该当前步骤的精确成功/失败/中断收据；收据只能写入当前 wait，施法期间仍不得推进游标，恢复后的健康业务帧再按原确认或重试路径处理。
- **1.5.4 窗口冷却重入覆盖**：旧窗口已离开后，新代次尚未派发且自身 CD 与官方窗口冲突时，只能保留唯一 capture 等待明确可用，不得因此消费新代次或建立不可复检的 departure lock。该窄场景允许窗口排首复用同一个 observation-only capture；必须按不同健康 runtime cycle 仅复检窗口，CD/UNKNOWN 不派发，ready 后重新预检并按原序建链。capture 的组/配置/代次、四帧恢复 handoff、跨组忽略和生命周期清理仍统一。已经消费或硬终止的离开锁不得因冷却恢复而自动解除；施法/引导/蓄力期间仍不复检、不建链、不推进。
...
多技能与 castsequence 宏保持既有正文匹配和 BindingToken 兼容；宏按钮状态只有通过共享 `IsSpellActionStateTrusted` 的单技能正文身份校验才可用于该技能冷却/可用性采样。复合宏按精确步骤 SpellID 采样与确认，不推测宏游标，也不以其他技能成功确认当前步骤。通用单饰品槽位宏继续按装备槽位及锁定 ItemID 校验。

## 1.5.7 饰品宏身份覆盖

当前动作栏明确返回 macro/item 子类型时，其 id 不得作为 numeric macro index 或代表 SpellID 使用；须由当前动作条名称、唯一当前宏索引与已解析物品/槽位正文恢复身份。没有该子类型的既有同索引有界读取规则保持不变；不得用宏列表存在、图标或无名称猜测授权。
...
- `simple`：首次派发只接受明确自身可用（`OWN_READY`）及其公开 GCD 时序；明确自身 CD 或 UNKNOWN 均不得以 GCD 状态降级为候选。绑定/装备身份仍有效的 CD/UNKNOWN 步骤可作为未越过位置的候补，只有位于当前配置游标之后并重新取得明确可用证据时才可按原位置补入；已越过步骤永久失效，剩余步骤相对顺序不变。
- `focused`：任何已启用可选步骤存在明确自身 CD、无绑定、未装备、装备变化或其他硬失效时不创建计划；单帧资源不足可进入运行期稳定确认，通用临时不可用必须保留原步骤等待恢复。
- 单帧资源不足或通用不可用不得删除可选注入；明确资源不足仍需两个不同共享业务快照稳定确认。通用 `usable=false` 且 `notEnoughResource=false` 在静止时标记为 `TEMPORARILY_UNUSABLE`，运行期输出 `BindingToken=0` 的 `burst_step_wait_usable`；玩家移动时，该布尔值只作诊断，不得否决已经由明确自身可用证据建立的派发资格。冷却来源不确定不得首次派发；只有当前 `WAIT_CONFIRM` 步骤已经以明确 `OWN_READY` 进入派发后，后续短暂 UNKNOWN 才可保持同一步共享节奏。普通读条、引导、蓄力期间必须在规则选择、候补扫描和步骤采样前 observation-only 暂停，不得改变 plan/group/stepIndex；明确自身 CD、绑定/装备变化与生命周期硬终止仍优先。噬灭虚空变形的 `usable=false/false` 兼容仍受其专用范围约束。
- 所有可选步骤均不可用时原则上不创建计划；仅 `simple` 中位于窗口之后、绑定/装备身份有效的 CD/UNKNOWN 候补可允许先建立窗口计划，以便在游标越过前取得明确可用证据后按原位置加入。位于窗口之前的候补不得因此重获机会。
...
- UNKNOWN、图标灰度、泛 GCD、单个失败事件、Buff、资源/目标状态不得作为首次派发、跳过或成功证据。当前 `WAIT_CONFIRM` 精确 SpellID/等效 SpellID 的匹配 `UNIT_SPELLCAST_FAILED`、`UNIT_SPELLCAST_FAILED_QUIET` 或 `UNIT_SPELLCAST_INTERRUPTED` 收据只为已锁定可选步骤建立两帧无 Token 隔离与同一步重试，不得按次数删除该步骤；移动只记录诊断，不建立持续等待，隔离结束后仍按共享节奏派发当前步骤。仅窗口步骤仍允许两个匹配失败形成有界活性释放；不得用候选帧数、固定睡眠或泛超时替代该精确失败证据。
