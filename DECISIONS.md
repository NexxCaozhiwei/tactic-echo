# 1.1.5 决策：HUD 点击路由保护与 TEK 修饰键手动接管
...
1.1.5 不重新开启自动打断。Defaults、Normalize 与 `AutoReaction:Evaluate()` 的 `auto_interrupt_suspended` 是当前生产边界；读条、钢条、事件和冷却证据只服务于只读提示、高亮、诊断与 HUD 人工点击。
