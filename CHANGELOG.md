...
- **噬灭资源不足不再卡校验**：用户明确授权对已验证动作栏来源的可选注入步骤读取 `IsUsableAction` 的公开布尔结果；修复共享快照把明确 `false` 折叠为 `nil` 的 Lua 布尔归一化问题。仅在 `usable=false` 且 `notEnoughResource=true` 时，`simple` 跳过该注入并继续、`focused` 不创建或释放计划。窗口步骤、UNKNOWN、具体资源数值、BindingToken、TEAP 与 TEK 均不受影响。
