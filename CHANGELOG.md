...
- **实机原因**：macro 32 item 被误当成第 32 个账号宏，导致角色饰一 /use 13 的来源丢失。
- **修复**：优先识别 item 子类型，通过当前动作条名称、唯一宏索引与正文恢复；保留无子类型的原索引路径和指定物品宏兼容。
- **验证边界**：同名歧义、无名称、双饰品槽位和无按键仍关闭派发；不修改玩家宏或输入协议。

# 1.5.6 — 自动注入复合宏状态隔离

- **保留宏兼容**：多行 cast、单饰品槽位 use、castsequence 继续通过已验证动作条宏与真实 BindingToken 派发，不改写宏。
- **修复状态串用**：多技能/序列宏的按钮状态不能代替当前步骤的精确技能冷却或资源状态，避免误跳过、等待或自身 CD 被宏按钮 GCD 覆盖。单技能宏继续支持按钮采样。
- **饰品与确认回归**：验证通用 /use 13 的索引和唯一名称恢复、装备身份门控，以及宏内其他技能的成功不推进当前步骤。用户反馈的饰品现象仍需运行日志定位。

# 1.5.5 — HUD 数字清晰与模式、注入提示常驻

- **冷却数字**：转盘透明度独立控制，原生准确倒计时提升至文字层，并同步清理隐藏状态。
- **常驻模式**：HAD/LCC 与运行状态同时展示，暂停、待命及引导时仍能辨认当前模式，增大字号并区分颜色。
- **LCC 保留队列**：自动注入关闭时保留已启用组的只读技能与冷却卡片，不显示执行中的活动组高亮；自动执行门控不变。

# 1.5.4 — 窗口冷却重入可恢复等待

- **修复新一轮窗口卡住**：同一战斗曾确认过的窗口在新进入边沿仍被采样为自身 CD 时，保留唯一 capture 等待可用，不再把尚未派发的新代次写成已消费或建立离开锁。
- **推荐不变也能恢复**：在不同健康业务快照中只复检当前窗口；取得明确自身可用证据后重新预检并按原顺序建立计划，无需额外一次官方推荐离开/进入。等待期间 CD/UNKNOWN 均不派发，同一 cycle 不重复采样。
- **所有顺序共享同一等待**：窗口前置注入与窗口排首均复用唯一 capture/Coordinator；窗口排首的重入等待保留必要的暂停恢复 handoff，不新增状态机、事件、轮询或输入路径。
- **保持确认与防重放**：已经消费的窗口和硬失效终止仍沿用离开锁；施法/引导/蓄力期间不采样、不建链，当前步骤确认后才推进。冷却/充能回退仍需不同样本与至少 0.15 秒稳定证据。
- **诊断与测试**：导出等待状态、最近采样 phase/cycle/时间和采样次数；覆盖 CD→UNKNOWN→ready、充能恢复、两种模式/窗口位置、完整顺序、防重放、施法保护、跨组、暂停 handoff 和生命周期清理。离线验证不替代客户端实测。

# 1.5.3 — 施法软暂停保留成功收据

- **修复奥术涌动后卡链**：已派发读条技能进入施法期 `SOFT_PAUSED` 时，若暂停前状态为 `WAIT_CONFIRM`，仍接收当前精确 SpellID 的成功事件；恢复后的首个健康帧据此确认当前步骤并继续大法师之触。
- **失败收据语义一致**：同一软暂停包装状态也接收当前步骤的精确失败/中断收据，继续使用既有两帧无 Token 隔离和同一步重试，不会推进或删除步骤。
- **顺序边界不变**：只扩展已锁定步骤的事件收据状态，不扩大技能等效匹配、不使用 Buff/GCD 作为成功证据，也不允许后续步骤越过当前步骤。
- **实测证据**：1.5.2 日志中的计划 3 在奥术涌动 `365350` 成功时处于 `SOFT_PAUSED`，成功事件未进入 AutoBurst，随后持续停在第 2 步；本版本为该精确竞态增加回归覆盖。

# 1.5.2 — 严格注入游标与施法期窗口栅栏

- **施法期不抢窗口**：普通施法、引导或蓄力期间仍不选择规则、不建计划、不扫候补；若官方推荐恰为已启用组的窗口，则只发布 `BindingToken=0` 的 observation-only 栅栏，避免官方主链提前派发窗口。
- **确认后才前进**：当前步骤必须由精确成功事件、稳定自身非 GCD 冷却开始或充能减少确认后，配置游标才向前移动；单纯发布候选或收到失败事件不会推进。
- **缺口等待新快照**：当前步骤确认后若下一活动步骤前存在 CD/UNKNOWN 候补，同一 runtime cycle 不得直接越过；必须等待新的业务快照，再按原 `configuredIndex` 接纳已明确可用的步骤。
- **过去步骤永久失效**：候补到达自身配置位置仍为 CD/UNKNOWN 时只越过一次，之后即使恢复也不得插到已派发步骤之前；计划不会重建、重排或重放前缀。
- **冷却证据收紧**：窗口与可选步骤的自身 CD 跳过证据统一要求至少两个不同业务样本并相隔不少于 0.15 秒，防止瞬时 API 冲突把可用技能误判为已冷却。
- **回归覆盖**：新增 `A→W→B→C` 的中段候补恢复、到位过期不回插、施法期窗口栅栏与同快照禁止跨缺口测试；TEAP/TEK 协议和唯一输入路径不变。

# 1.5.1 — 自动注入正向准入与施法保护

- **正向可用准入**：技能首次进入派发必须已由可信动作条、冷却/充能采样明确判定为自身可用；`UNKNOWN`、opaque 冷却和仍在自身 CD 的技能不再通过公开 GCD 状态降级成候选。
- **已接纳步骤持续派发**：当前步骤一旦以明确 `OWN_READY` 进入 `WAIT_CONFIRM`，后续冷却读数短暂变为 UNKNOWN 时仍可按共享 TEK 节奏继续；明确观察到自身 CD 后立即停止重派并等待稳定成功确认。
- **移动不再锁链停派**：移动造成的失败只保留诊断与既有两帧重试隔离，不再进入持续 `burst_step_wait_movement`；隔离结束后仍尝试当前步骤，不让后续步骤越过。
- **施法过程零干扰**：普通施法、引导和蓄力在规则选择、计划变更与候补扫描之前进入 observation-only 暂停，保持当前 plan、group 和配置游标不变。
- **单向后续候补**：未来 CD/UNKNOWN 步骤只有在游标越过前取得明确可用证据才按原配置位置加入；已经过去的步骤永久失效，不得回插、重建或重放前缀。
- **诊断增强**：安全标量导出增加未来候补数量、移动可用性兼容与移动状态，未新增输入通道或协议字段。

# 1.5.0 — 稳定注入链与原生 HUD 提示

- **稳定版注入计划**：正式冻结单次建链、单向配置游标和后续冷却候补补入；执行过程中不重建或重放已越过的链前缀。
- **锁定步骤可靠派发**：已进入活动链的可选步骤不再因匹配失败次数被删除；移动失败期间保持无 Token 等待，停下后继续同一步。
- **原生 HUD 提示**：技能、装备饰品和普通物品的 HUD 悬停统一交给暴雪原生提示接口，不再显示内部测试与诊断字段。
- **安全架构不变**：继续保持唯一 AutoInjectionCoordinator、唯一 OrderedPlan，以及 BindingToken → TEAP → TEK 的既有输入路径和全部安全门禁。
- **版本里程碑**：`1.4.13` 至 `1.4.15` 的实测修复合并定版为 `1.5.0`。

# 1.4.15 — HUD 恢复暴雪原生提示信息

- **移除测试信息**：HUD 悬停不再追加类型、状态、原因、现实按键、冷却来源、转盘渲染、注入组和手动点击等 Tactic Echo 诊断行。
- **原生技能提示**：技能卡直接按 SpellID 调用暴雪技能提示框，与普通技能按钮的信息保持一致。
- **原生物品提示**：已装备饰品优先按角色装备槽调用暴雪物品提示；其他物品按 ItemID 显示，兼容客户端的物品链接回退。
- **安全边界不变**：此次只调整 HUD 鼠标悬停展示，不改变自动注入计划、冷却判断、BindingToken、TEAP、TEK 或 HUD 手动点击代理。

# 1.4.14 — 单次锁链、单向游标与后续候补补入

- **一次建链**：官方窗口首次命中时只创建一个 OrderedPlan；初始可用步骤组成活动链，不再因运行期状态变化重新生成 `A→W→B→C` 前缀。
- **冷却候补**：初始明确自身 CD、但绑定与装备身份仍有效的可选步骤保留原始配置序号。计划运行期间持续复检，只有位于当前游标之后的步骤恢复后才能按原位置补入。
- **只向前推进**：已经越过的候补永久过期。例如初始锁定 `W→B` 后 A、C 同时恢复，只追加 C；A 不回填，W/B 也不重放。
- **锁定步骤不再失败熔断**：可选步骤一旦进入活动链，匹配施法失败只建立两帧无 Token 隔离并重试同一步，不再以两次失败跳过；窗口步骤仍保留有界失败释放，避免错误窗口绑定永久占用。
- **移动等待**：精确失败发生时若玩家正在移动，当前步骤进入 `burst_step_wait_movement`，移动期间不发布 Token；停下后再经过既有隔离帧恢复派发。
- **审计字段**：新增候补初始数、待定数、补入数、过期数、配置长度、补入/过期事件与移动失败状态，映射导出仍只包含安全标量。
- **架构边界不变**：继续复用唯一 Coordinator、唯一 OrderedPlan、现有 RuntimeSnapshot/SignalFrame 刷新以及 BindingToken→TEAP→TEK 输入路径；未新增事件、OnUpdate 或第二派发器。

# 1.4.13 — 移动等临时不可用不再丢失注入步骤

- **实际动作槽可用性闸门**：可选技能注入同时读取技能与已验证动作槽的公开可用性；动作槽是 TEK 将实际按下来源的派发权威，不再因技能探针先返回而遗漏动作槽 `false`。
- **临时不可用只等待**：`usable=false` 且没有明确资源不足、普通施法/GCD 延后或自身 CD 证据时，步骤进入 `TEMPORARILY_UNUSABLE`，保持原 plan、group、stepIndex 和顺序，输出 `BindingToken=0` 的 `burst_step_wait_usable`；恢复可用后自动回到既有 GCDGate/TEK 路径。
- **移动竞态失败撤票**：已派发步骤若收到匹配失败后观察到临时不可用，该次失败从“两次失败释放”证书中撤销；恢复后仍经过两帧 observation-only 隔离并建立新的 `dispatchAttempt`，不会停下后立即丢弃奥术涌动等读条技能。
- **硬失效优先**：明确自身非 GCD 冷却、绑定/装备变化、资源不足稳定证据、脱战、死亡与切图仍按原规则释放或重置，临时可用性不得遮蔽自身 CD 并形成永久等待。
- **架构不变**：未新增移动轮询、技能白名单、事件、状态机或输入通道；BindingToken、TEAP v3、TEK 门禁、成功确认和唯一 AutoInjectionCoordinator/OrderedPlan 保持不变。
- **回归覆盖**：新增动作槽 `false` 覆盖技能 `true`、初始等待后恢复、失败后临时不可用撤票与恢复 TEK 尝试、自身 CD 不被临时状态遮蔽等行为测试。
...
- **有界自适应预算**：复检预算调整为受限 `SpellQueueWindow + 0.75s`，最少 0.85 秒、最多 1.20 秒；它只决定 observation-only capture 的最长持有时间，不替代 GCDGate、冷却状态或成功确认。
- **资格不放宽**：仍只接受序列第一步、窗口之前、Resolver 已验证并由精确直接动作槽位报告自身冷却的技能；必须等同一槽位明确 ready 才能创建原顺序 plan。
- **派发边界不变**：等待期间继续保持 `BindingToken=0`、不创建 plan、不发送 TEAP 动作；窗口离开、脱战、绑定/规则变化和预算到期仍按原 simple/focused 逻辑 fail-closed。
- **回归覆盖**：行为测试新增 0.90 秒晚到 ready 仍恢复 `31884 → 窗口`，以及超过 1.20 秒持续冷却安全释放的边界。

# 1.4.8 — 前置注入冷却结束边沿复检

- **复仇之怒边沿稳定性**：官方窗口在 31884 上次成功后的最后约半秒命中时，不再把该步骤永久移出本轮；可信直接动作槽位进入有界 pre-window capture 复检，明确 ready 后仍按 `31884 → 窗口` 执行。
- **仅观察、不提前派发**：边沿复检期间保持 `observationOnly=true`、`BindingToken=0`，不创建 plan、不发送 TEAP Burst 动作，也不把 HUD、DurationObject 或图标状态作为资格证据。
- **严格适用范围**：只接受序列第一步、窗口之前、Resolver 已验证且具备精确自身冷却证据的直接技能按钮；宏、饰品、窗口后步骤、不可信槽位与 UNKNOWN 均不能进入。
- **有界失败关闭**：复检预算由受限 `SpellQueueWindow` 加确认余量计算并封顶 0.70 秒；持续冷却或预算到期恢复原 simple/focused 预检，窗口/绑定/战斗状态变化继续沿用既有 capture 终止保护。
- **审计与回归**：新增边沿开始、明确 ready、到期事件，以及 capture/预检纯标量字段；行为测试覆盖 ready 恢复原顺序和超时无候选两条路径。

# 1.4.7 — 惩戒骑注入冷却冲突修复

- **可信动作槽位就绪纠正**：Resolver 已验证的直接默认动作槽位若给出可解释的 `0/0` 且明确不在 GCD，即使 Retail 同时残留 `isActive=true`，也可纠正声明 SpellID 的旧基础冷却，避免 31884 实际已恢复却被预检判为 `spell_cooldown_active`。
- **安全范围不扩张**：纠正必须同时满足直接槽位与可信动作栏身份；宏、间接来源、不可信槽位、共享 GCD、未知或非零冷却均不能借此进入自动注入链。
- **冲突审计可见**：步骤观察、预检排除、日志和安全映射导出新增纯标量 ready-conflict 字段，不导出原始受保护冷却数值。
- **行为回归**：新增矛盾 `0/0 + isActive=true` 的可信直接槽位通过用例，以及相同观察在非直接来源下仍保持自身冷却的拒绝用例。
- **架构不变**：唯一 Coordinator、唯一 OrderedPlan、不同样本稳定确认、BindingToken、TEAP v3、TEK 和脱战硬门控保持不变。
